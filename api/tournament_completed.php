<?php
	...
	if(isset($_POST) && !empty($_POST)){
		...
		if(mysqli_num_rows($tokenQry) == 1){
			$sql = "SELECT t.*, g.gamename, gp.gameplatform, tt.tournamenttype FROM tournament AS t INNER JOIN game AS g ON g.id=t.game_id INNER JOIN game_platform AS gp ON gp.id=t.game_platform_id INNER JOIN tournament_type AS tt ON tt.id=t.tournament_type_id WHERE t.isActive=2 ORDER BY t.match_datetime DESC";
			...
			if (mysqli_num_rows($rs) > 0) {
				...
				$response['message'] = 'No Ongoing tournaments at the moment!';
			}
		}else{
			$response['status'] = 404;
