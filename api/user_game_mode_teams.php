		...
		if(mysqli_num_rows($tokenQry) == 1){
			...
			if (mysqli_num_rows($rs) > 0) {
				...
				while($row = mysqli_fetch_assoc($rs)){
					...
					$gameModeSql = "SELECT gm.team_name FROM user_game_mode_data AS gm INNER JOIN game AS g ON g.id=gm.game_id INNER JOIN tournament_type AS tt ON tt.id=gm.tournament_type_id WHERE gm.game_id=$game_id AND gm.member_user_id=$user_id AND gm.request_accepted=1 AND gm.tournament_type_id=".$row['id'];
					$gameModeRes = mysqli_query($con,$gameModeSql);
					while($gameModeRow = $gameModeRes->fetch_assoc()){
						$row['teams'][] = $gameModeRow;
					}
