<?php
	...
	if(isset($_POST) && !empty($_POST)){
		...
		if(mysqli_num_rows($tokenQry) == 1){
			...
			$fetchQry = fetchFromTable("withdrawal_history",null,$conditionString);
			if (mysqli_num_rows($fetchQry) > 0) {
				...
				$response['status'] = 1;
				$response['message'] = 'All withdrawal requests.';
			}else{
				$response['message'] = 'No withdrawal requests at the moment!';
			}
		}else{
			$response['status'] = 404;
