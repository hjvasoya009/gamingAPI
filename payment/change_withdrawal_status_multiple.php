<?php
    ...
    if(isset($_POST) && !empty($_POST) && isset($_POST['withdraw_status'])){
	    ...
	    foreach($checked_data as $keyToUpdate){
	    	...
	    	$conditionString = "withdraw_status != $withdraw_status AND id=$withdrawal_id";
			...
			if(mysqli_num_rows($withdrawHistoryQry) == 1){
				...
				if(mysqli_num_rows($userQry) == 1){
					...
					if(!$error){
						...
						if(!$transaction_error){
					        ...
					        $insertResult = editRecordFromTable('withdrawal_history', $data, $keyToUpdate);
					        if (!$insertResult['mysqli_error']) {
				...
				$arrResult['error'] = true;
				$arrResult['message'] = " Failed to Proceed. Some of the selected withdraw request is already $withdraw_status_text.";
				break;
			}
		}
