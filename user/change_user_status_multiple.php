<?php
    ...
    if(isset($_POST) && !empty($_POST) && isset($_POST['user_status'])){
	    ...
	    foreach($checked_data as $keyToUpdate){
	    	
	    	$data = new \stdClass();
	        $data->user_status 		 = $user_status;
	        $data->suspension_reason = $suspension_reason;
	        $data->updated_at 		 = $datetime;
			
	        $insertResult = editRecordFromTable('users', $data, $keyToUpdate);
		...
		if ($arrResult['error']) {
	        ...
	        $con->commit();
	        if($user_status == 1){
				$arrResult['message'] = " Successfully Reactivated ";
			}elseif($user_status == 2){
				
				$arrResult['message'] = " Successfully Suspended ";
				
				foreach($checked_data as $keyToUpdate){
	    	
			    	$conditionString = "id=$keyToUpdate AND user_status=2";
					$userQry = fetchFromTable("users",null,$conditionString);
						
					$userRow = mysqli_fetch_assoc($userQry);
					
					$to      = $userRow['email'];
					$subject = "Gaming Akhada user suspension.";
			...
			<body style="margin: 0; padding: 0; font-family: &quot; HelveticaNeueLight&quot;,&quot;HelveticaNeue-Light&quot;,&quot;HelveticaNeueLight&quot;,&quot;HelveticaNeue&quot;,&quot;HelveticaNeue&quot;,Helvetica,Arial,&quot;LucidaGrande&quot;,sans-serif;font-weight: 300; font-stretch: normal; font-size: 14px; letter-spacing: .35px; background: #EFF3F6; color: #333333;">
							...
							<td style="border: 0 none; border-collapse: separate; vertical-align: middle;" valign="middle">
								<table align="center" border="1" cellpadding="0" cellspacing="0" class="c-f1bud4" style="border: 0 none; border-collapse: separate; width: 100%;" width="100%">
									<tbody>
			                          ...
			                          valign="middle">
																'.$userRow['first_name'].' '.$userRow['last_name'].' ('.$userRow['username'].'), Your account has been suspended on Gaming Akhada.
															</td>
														</tr>
														<tr style="border: 0 none; border-collapse: separate;">
			                          ...
			                          valign="middle">
																Below is the reason for your suspension.
															</td>
														</tr>
														<tr style="border: 0 none; border-collapse: separate;">
														...
														<tr style="border: 0 none; border-collapse: separate;">
															<td style="border: 0 none; border-collapse: separate; vertical-align: middle;" valign="middle">
																<span class="c-1w4lcwx">
																	You receive this email because we found unusual activity on your Gaming Akhada account.
																</span>
															</td>
														</tr>
													</tbody>
				...
				$arrResult['message'] = "  Successfully Deactivated  ";
			}
	    }
	}
